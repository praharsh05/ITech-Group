...
def forum(request):
   ...
   if request.method == 'POST':
       ...
       cursor.execute(sql, [username, request.POST['content'],request.POST['title']])
...
def myaccount(request):
   ...
   if request.method == 'POST':
       ...
       if user_info:
           sql = 'UPDATE user_info SET intro=%s,FirstName=%s,LastName=%s WHERE UserID = %s '
           cursor.execute(sql,[request.POST.get('intro'),request.POST.get('f_name'),request.POST.get('l_name'),username])
...
def creat_comment(request):
   ...
   cursor.execute(sql, [username, request.POST['comment'],request.POST['thread_id']])
   new_url = '/forum/forum/?id={}'.format(request.POST.get('id'))
   print(new_url)
   return redirect(new_url)
